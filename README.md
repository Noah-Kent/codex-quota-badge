...
> 当前为开源预览版。Release 中的 `.dmg` 使用本地临时签名，尚未经过 Apple 公证。macOS 的安全提示表示系统无法验证发布者身份，**不等同于**系统认定本项目恶意或不安全；请始终从本仓库下载并自行审查源码。

## 功能

- 双行紧凑显示 `5H` 和 `7D` 剩余配额；
- 点击后用彩色进度条查看两个周期的剩余配额、重置时间和最后更新时间；
- 提供“立即刷新”“隐藏详情”和“完全退出”；
...
从 [GitHub Releases](../../releases/latest) 下载预览 `.dmg`。打开后，将应用拖入“应用程序”文件夹即可。

该预览包尚未使用 Developer ID 签名，也未完成 Apple 公证，因此首次打开时 macOS 可能阻止启动或显示“无法验证开发者”等安全提示。这反映的是**发布身份尚未被 Apple 验证**，而不是对软件安全性的判定。请先确认文件来自本仓库；不要为了安装而关闭 Gatekeeper 或降低系统整体安全设置。

如果你不希望处理此类下载安全提示，推荐从源码在自己的 Mac 上构建。它不会携带互联网下载文件的隔离标记，通常不会出现该提示。
...
- 当前 Release 为临时签名、未公证的预览包；安全提示代表发布身份未验证，并不等同于恶意软件结论。希望避开提示的用户可从源码自行构建。
